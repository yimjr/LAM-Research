...
def write_card(card_id: str, title_zh: str, title_en: str, body_zh: list[str], body_en: list[str]) -> None:
    card_dir = ROOT / "results/hypothesis_cards_hdbscan"
...
def main() -> None:
    ...
    result_dir = ROOT / "results/discovery_hdbscan"
    table_dir = result_dir / "tables"
    result_dir.mkdir(parents=True, exist_ok=True)
    table_dir.mkdir(parents=True, exist_ok=True)
    adata = ad.read_h5ad(input_path)
    state_columns = [f"state_{name}" for name in STATE_PROGRAMS]
    micro_columns = [f"microenv_{name}" for name in MICROENVIRONMENT_PROGRAMS]
    missing_scores = [column for column in [*state_columns, *micro_columns] if column not in adata.obs]
    if missing_scores:
        raise RuntimeError(
            "The HDBSCAN fork expects the existing core-reproduction h5ad to contain "
            f"precomputed state scores; missing columns: {missing_scores}"
        )
    # Reuse the score columns already present in the core-reproduction object.
    # This branch changes only the heterogeneity method and does not rerun the
    # original Scanpy scoring or alter the input h5ad.
    statuses = {name: {"status": "reused_precomputed_score", "column": f"state_{name}"} for name in STATE_PROGRAMS}
    micro_statuses = {name: {"status": "reused_precomputed_score", "column": f"microenv_{name}"} for name in MICROENVIRONMENT_PROGRAMS}
    ...
    hdbscan_rows = []
    hdbscan_cluster_rows = []
    primary_assignment = None
    primary_spec = None
    if len(candidate_cells) >= 10:
        raw_matrix = candidate_cells.to_numpy(dtype=float)
        # The primary setting is fixed in advance. The remaining settings
        # assess sensitivity to score scaling and density parameters rather
        # than selecting the most attractive result after seeing the output.
        hdbscan_specs = [
            {"score_space": "zscore", "min_cluster_size": 10, "min_samples": 5, "primary": True},
            {"score_space": "zscore", "min_cluster_size": 10, "min_samples": 10, "primary": False},
            {"score_space": "zscore", "min_cluster_size": 8, "min_samples": 5, "primary": False},
            {"score_space": "zscore", "min_cluster_size": 15, "min_samples": 5, "primary": False},
            {"score_space": "raw", "min_cluster_size": 10, "min_samples": 5, "primary": False},
            {"score_space": "raw", "min_cluster_size": 10, "min_samples": None, "primary": False},
        ]
        for spec in hdbscan_specs:
            matrix = raw_matrix if spec["score_space"] == "raw" else StandardScaler().fit_transform(raw_matrix)
            model = HDBSCAN(
            ...
            assigned = labels >= 0
            cluster_ids = sorted(set(labels[assigned].tolist()))
            silhouette = np.nan
            if len(cluster_ids) >= 2 and int(assigned.sum()) > len(cluster_ids):
                silhouette = float(silhouette_score(matrix[assigned], labels[assigned]))
            hdbscan_rows.append({
                "score_space": spec["score_space"],
                "min_cluster_size": int(spec["min_cluster_size"]),
                "min_samples": spec["min_samples"],
                "cluster_selection_method": "eom",
                "cells": int(len(candidate_cells)),
                "n_clusters": int(len(cluster_ids)),
                "assigned_cells": int(assigned.sum()),
                "noise_cells": int((~assigned).sum()),
                "noise_fraction": float((~assigned).mean()),
                "assigned_donors": int(adata.obs.loc[candidate_cells.index[assigned], "donor_id"].nunique()),
                "silhouette_assigned": silhouette,
                "primary_setting": bool(spec["primary"]),
            })
            for label in cluster_ids:
                ...
                row = {
                    ...
                    "cluster": int(label),
                    "cells": int(mask.sum()),
                    "donors": int(adata.obs.loc[candidate_cells.index[mask], "donor_id"].nunique()),
                    "donor_ids": ";".join(sorted(adata.obs.loc[candidate_cells.index[mask], "donor_id"].astype(str).unique())),
                }
                for column in state_columns:
                    row[f"mean_{column}"] = float(candidate_cells.loc[mask, column].mean())
                hdbscan_cluster_rows.append(row)
            if spec["primary"]:
                primary_assignment = labels
                primary_spec = spec
    pd.DataFrame(hdbscan_rows).to_csv(table_dir / "lamcore_state_heterogeneity_hdbscan.csv", index=False)
    pd.DataFrame(hdbscan_cluster_rows).to_csv(table_dir / "lamcore_state_hdbscan_cluster_summary.csv", index=False)
    if primary_assignment is not None and primary_spec is not None:
        assignment = adata.obs.loc[candidate_cells.index, ["sample_id", "donor_id", "condition", "assay"]].copy()
        assignment["hdbscan_cluster"] = primary_assignment
        assignment["hdbscan_is_noise"] = primary_assignment < 0
        assignment["hdbscan_score_space"] = primary_spec["score_space"]
        assignment["hdbscan_min_cluster_size"] = primary_spec["min_cluster_size"]
        assignment["hdbscan_min_samples"] = primary_spec["min_samples"]
        assignment.to_csv(table_dir / "lamcore_state_hdbscan_assignments.csv")
    heterogeneity = hdbscan_rows
    ...
    external = [
        {"dataset": "GSE122960_normal_lung", "status": "not_run_in_hdbscan_branch", "reason": "This branch isolates the state-heterogeneity method and reuses precomputed core scores."},
        {"dataset": "GSE118180_wildtype_uterus", "status": "not_run_in_hdbscan_branch", "reason": "This branch isolates the state-heterogeneity method and reuses precomputed core scores."},
    ]
    ...
    primary_result = next((row for row in hdbscan_rows if row.get("primary_setting")), {})
    primary_hdbscan_summary_zh = (
        f"主设定在 {primary_result.get('cells', 0)} 个候选细胞中识别出 "
        f"{primary_result.get('n_clusters', 0)} 个密度簇，分配 "
        f"{primary_result.get('assigned_cells', 0)} 个细胞；"
        f"{primary_result.get('noise_cells', 0)} 个细胞被标记为噪声，"
        f"覆盖 {primary_result.get('assigned_donors', 0)} 个 donor。"
    )
    primary_hdbscan_summary_en = (
        f"Under the primary setting, HDBSCAN identified {primary_result.get('n_clusters', 0)} "
        f"density clusters among {primary_result.get('cells', 0)} candidates, assigned "
        f"{primary_result.get('assigned_cells', 0)} cells, and marked "
        f"{primary_result.get('noise_cells', 0)} as noise across "
        f"{primary_result.get('assigned_donors', 0)} donors."
    )
    card1_zh = [
        ...
        "",
        f"## HDBSCAN 结果\n{primary_hdbscan_summary_zh}当前主设定中的非噪声细胞只来自 LAM3 和 LAM4；LAM1 与 LAM2 没有被分配到密度簇。因此，HDBSCAN 没有支持一个覆盖四个 donor 的稳定离散状态；噪声标签表示状态空间中未形成足够密度的区域，不表示细胞没有生物学意义。",
        "",
    ...
    card1_en = [
        ...
        "",
        f"## HDBSCAN result\n{primary_hdbscan_summary_en} The primary non-noise cells came only from LAM3 and LAM4; no LAM1 or LAM2 candidate was assigned to a density cluster. Thus HDBSCAN did not support a stable discrete state spanning all four donors. A noise label means that a point was not assigned to a sufficiently dense region in this state space, not that the cell lacks biological meaning.",
        "",
    ...
    manifest = {
        "script": "scripts/explore_lam_hypotheses_hdbscan.py",
        "input": str(input_path.relative_to(ROOT)),
        "output_dir": str(result_dir.relative_to(ROOT)),
        "method": "HDBSCAN replacement for the original KMeans state-heterogeneity exploration",
        "candidate_definition": "lamcore_candidate_author_style from the existing core-reproduction h5ad; no candidate labels were redefined here",
        "state_columns": state_columns,
        "parameter_grid": hdbscan_rows,
        "noise_interpretation": "HDBSCAN label -1 is an unassigned/noise point in state space, not evidence that the cell is biologically irrelevant.",
        "comparison_baseline": "results/discovery/tables/lamcore_state_heterogeneity_exploration.csv",
        "original_script_unchanged": True,
        "input_h5ad_not_overwritten": True,
    }
    (result_dir / "hdbscan_analysis_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False))
