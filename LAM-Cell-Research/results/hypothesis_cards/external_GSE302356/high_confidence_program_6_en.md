...
`program_6` emerged in pooled NMF of the `high_confidence` candidate pool. Leading genes are: MALAT1, SGMS2, PLCG2, RASAL2, TRIO, MAP2, GLIS3, PTPN1, TOP1, GPRC5D-AS1, MYOF, DGKD.
...
Strongest known-program match: IL6_AT2_repair (microenvironment; overlap=0). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
