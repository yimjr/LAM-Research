...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：MALAT1, DST, CACNA1C, CALD1, PALLD, RORA, ABI3BP, FTX, FBXL7, LHFPL6, ZFPM2, TCF4。
...
最强的已知程序匹配：HOX_PBX (LAM-shared; overlap=2)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
