...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：MALAT1, CFAP299, AGBL4, RFX3, TMEM232, RP1, CHST9, MAPK10, SPAG16, DNAH12, LRRIQ1, STK33。
...
最强的已知程序匹配：hormone_related (LAM-shared; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
