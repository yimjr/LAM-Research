...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_6`。当前排名靠前的基因为：MALAT1, SGMS2, PLCG2, RASAL2, TRIO, MAP2, GLIS3, PTPN1, TOP1, GPRC5D-AS1, MYOF, DGKD。
...
最强的已知程序匹配：IL6_AT2_repair (microenvironment; overlap=0)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
