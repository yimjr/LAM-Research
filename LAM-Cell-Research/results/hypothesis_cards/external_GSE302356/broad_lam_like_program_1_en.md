...
`program_1` emerged in pooled NMF of the `broad_lam_like` candidate pool. Leading genes are: MALAT1, MAP2, RASAL2, TRIO, NAV2, PLCG2, SCEL, GPRC5D-AS1, DST, NEDD9, GPRC5A, AC027288.3.
...
Strongest known-program match: hormone_related (LAM-shared; overlap=0). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
