...
`program_3` emerged in pooled NMF of the `high_confidence` candidate pool. Leading genes are: MALAT1, DST, CACNA1C, CALD1, PALLD, RORA, ABI3BP, FTX, FBXL7, LHFPL6, ZFPM2, TCF4.
...
Strongest known-program match: HOX_PBX (LAM-shared; overlap=2). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
