...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_5`。当前排名靠前的基因为：MALAT1, PLXDC2, ZEB2, FKBP5, MRC1, FMNL2, ASAP1, ACSL1, CHST11, SLC8A1, DOCK8, ELMO1。
...
最强的已知程序匹配：CORE3_identity (LAMCORE-specific; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
