...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_2`。当前排名靠前的基因为：MALAT1, RPS8, RPL3, RPS3, RPS6, RPL5, TXNIP, H3F3B, SERF2, CD63, IGFBP7, LGALS1。

## donor 重复性

逐 donor 独立程序发现后，达到当前基因重叠标准的 donor：LAM32。这不是“在 pooled 模型中打分”，而是独立 donor 发现的初步匹配；若为空，说明目前没有足够证据称其跨 donor 重复。

## 与已知框架的关系

最强的已知程序匹配：mTOR_translation (TSC-tumor-shared; overlap=6)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
