...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_2`。当前排名靠前的基因为：MALAT1, CLDN5, IFI27, PECAM1, MT1M, SPTBN1, IFITM3, CAVIN2, RGCC, MT1E, HYAL2, TCF4。
...
最强的已知程序匹配：CORE1 (LAMCORE-specific; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
