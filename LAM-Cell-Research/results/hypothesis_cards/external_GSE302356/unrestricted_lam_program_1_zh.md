...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：MALAT1, SFTPC, ATP11A, SNX25, ACOXL, RBPMS, MICAL2, GPHN, MAGI3, CADM1, DIAPH2, MTUS1。
...
最强的已知程序匹配：IL6_AT2_repair (microenvironment; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
