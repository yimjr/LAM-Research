...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：MALAT1, MAP2, RASAL2, TRIO, NAV2, PLCG2, SCEL, GPRC5D-AS1, DST, NEDD9, GPRC5A, AC027288.3。
...
最强的已知程序匹配：hormone_related (LAM-shared; overlap=0)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
