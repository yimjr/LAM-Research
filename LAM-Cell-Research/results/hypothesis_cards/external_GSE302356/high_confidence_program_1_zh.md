...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：MALAT1, SFTPC, ACOXL, SNX25, MAGI3, PTPN13, AFF3, PTPRK, MTUS1, ATP11A, CADPS2, RAPGEF5。
...
最强的已知程序匹配：IL6_AT2_repair (microenvironment; overlap=2)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
