...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_2`。当前排名靠前的基因为：HOXA10, CRIP2, JUN, RPS3, C8ORF4, MYH11, TIMP1, MMP9, AC092580.4, KRT18, LGR5, ICAM1。
...
最强的已知程序匹配：ECM_remodeling (LAM-shared; overlap=3)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
