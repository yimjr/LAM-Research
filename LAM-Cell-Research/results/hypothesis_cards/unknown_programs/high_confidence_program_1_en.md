# LAM Research Hypothesis Card: high_confidence / program_1
...
`program_1` emerged in pooled NMF of the `high_confidence` candidate pool. Leading genes are: C1QA, CD300LG, LGR5, FIGF, SMAD2, RGCC, CYP19A1, RAPGEF4, HOXB7, COL3A1, MKI67, HMGA2.

## Donor reproducibility

Donors meeting the current top-gene matching rule after donor-wise discovery: LAM3. This is independent donor discovery evidence, not passive scoring of a pooled model; an empty value means that cross-donor support is currently insufficient.

## Relation to known frameworks

Strongest known-program match: protease_ECM_niche (microenvironment; overlap=5). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
