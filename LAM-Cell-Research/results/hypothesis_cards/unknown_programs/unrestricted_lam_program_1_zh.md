...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：TREM2, AGR3, IL6, GPNMB, LCN2, EGFL6, SELL, MYH11, SFTPA1, HMGA2, CCL2, DES。
...
最强的已知程序匹配：IS_inflammatory (treatment-associated; overlap=3)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
