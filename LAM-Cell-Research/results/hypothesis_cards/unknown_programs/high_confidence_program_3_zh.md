...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：PCNA, PCDH17, SOX2, TYR, IFI27, COL3A1, TCF4, IL6, HMGA2, DYNLRB2, LST1, HEG1。
...
最强的已知程序匹配：IL6_AT2_repair (microenvironment; overlap=3)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
