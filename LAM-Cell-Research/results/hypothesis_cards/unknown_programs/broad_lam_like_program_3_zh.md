...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：C1QA, CXCL14, ADRB1, LST1, MDK, CD300LG, NES, NGFRAP1, MYH11, NDRG1, TIMP1, MMP9。
...
最强的已知程序匹配：IL6_AT2_repair (microenvironment; overlap=5)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
