...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：NUPR1, MYL9, TGFBR1, ATP5E, ACTA2, ARHGDIB, TPM2, LITAF, RPS8, HLA-E, RHOB, TKT。
...
最强的已知程序匹配：lineage_uterine_smooth_muscle (lineage/reference; overlap=4)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
