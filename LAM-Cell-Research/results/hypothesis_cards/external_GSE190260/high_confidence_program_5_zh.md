...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_5`。当前排名靠前的基因为：HSP90AA1, STK17A, HLA-DQB1, CDC37, NFKBIA, RPLP0, SOD2, TREM2, RPS3, ARHGDIB, SIX1, EIF4B。
...
最强的已知程序匹配：HOX_PBX (LAM-shared; overlap=3)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
