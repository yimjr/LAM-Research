...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_4`。当前排名靠前的基因为：TOP2A, LUM, YBX1, COL6A2, SFTPA2, SSR1, ARHGDIB, CCL2, PI16, HMGN2, NUPR1, GLS。
...
最强的已知程序匹配：LAF_niche (microenvironment; overlap=4)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
