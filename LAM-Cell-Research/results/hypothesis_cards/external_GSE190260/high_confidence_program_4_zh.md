...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_4`。当前排名靠前的基因为：STK17A, ACTA2, MLANA, C7, IL32, TMSB10, HNRNPM, HSP90AB1, HSP90AA1, SMAD3, CDC37, RPLP0。
...
最强的已知程序匹配：mTOR_translation (TSC-tumor-shared; overlap=3)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
