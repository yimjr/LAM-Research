...
`program_4` emerged in pooled NMF of the `broad_lam_like` candidate pool. Leading genes are: TOP2A, LUM, YBX1, COL6A2, SFTPA2, SSR1, ARHGDIB, CCL2, PI16, HMGN2, NUPR1, GLS.
...
Strongest known-program match: LAF_niche (microenvironment; overlap=4). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
