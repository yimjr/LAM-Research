...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_5`。当前排名靠前的基因为：ARHGDIB, TMSB10, NUPR1, RPS3, ARF4, ATP5E, SLC2A3, SERP1, RPS8, TKT, SRRM1, SOD2。

## donor 重复性

逐 donor 独立程序发现后，达到当前基因重叠标准的 donor：LAM1164。这不是“在 pooled 模型中打分”，而是独立 donor 发现的初步匹配；若为空，说明目前没有足够证据称其跨 donor 重复。
