...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_2`。当前排名靠前的基因为：EGLN3, LUM, COL4A1, CITED2, ITM2B, ALG13, COX7C, HSP90AA1, CLEC7A, UBB, P4HB, IL32。
...
最强的已知程序匹配：normal_lung_interstitial (lineage/reference; overlap=3)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
