...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_5`。当前排名靠前的基因为：TAF1D, MDK, TYMS, MKI67, RBPMS, HLA-A, RORA, NHSL1, LYST, ARHGAP24, XIST, IKZF1。
...
最强的已知程序匹配：cell_cycle (LAM-shared; overlap=2)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
