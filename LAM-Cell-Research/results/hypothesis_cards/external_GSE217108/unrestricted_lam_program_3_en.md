...
`program_3` emerged in pooled NMF of the `unrestricted_lam` candidate pool. Leading genes are: TAF1D, RASA2, ITPR2, VMP1, PDE8A, PRKAG2, DENND1B, HLA-A, ZBTB38, LPP, TIMP1, ITGA9.
...
Strongest known-program match: protease_ECM_niche (microenvironment; overlap=2). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
