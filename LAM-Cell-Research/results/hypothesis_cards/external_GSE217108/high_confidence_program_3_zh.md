...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：KIF5B, PELI1, PICALM, IKZF1, JMJD1C, HOXD11, FHIT, CEMIP2, MYH9, ZSWIM6, SRGN, ACOXL。
...
最强的已知程序匹配：TGFbeta_fibroblast (microenvironment; overlap=2)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
