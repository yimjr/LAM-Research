...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：MALAT1, NHSL1, KCNMA1, FCHO2, CPM, SLC1A3, MSR1, MYO1E, CCDC88A, CTSB, FTX, RBM47。
...
最强的已知程序匹配：CORE2 (LAMCORE-specific; overlap=2)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
