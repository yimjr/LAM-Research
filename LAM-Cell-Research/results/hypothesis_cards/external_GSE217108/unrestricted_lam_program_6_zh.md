...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_6`。当前排名靠前的基因为：TAF1D, HOXD11, HLA-A, HLA-E, MEF2A, TMSB10, LPP, ATP1A1, MYO9A, PELI1, LYN, RABGAP1L。
...
最强的已知程序匹配：HOX_PBX (LAM-shared; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
