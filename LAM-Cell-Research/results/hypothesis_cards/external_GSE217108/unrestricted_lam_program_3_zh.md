...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_3`。当前排名靠前的基因为：TAF1D, RASA2, ITPR2, VMP1, PDE8A, PRKAG2, DENND1B, HLA-A, ZBTB38, LPP, TIMP1, ITGA9。
...
最强的已知程序匹配：protease_ECM_niche (microenvironment; overlap=2)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
