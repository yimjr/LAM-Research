...
`program_5` emerged in pooled NMF of the `unrestricted_lam` candidate pool. Leading genes are: TAF1D, MDK, TYMS, MKI67, RBPMS, HLA-A, RORA, NHSL1, LYST, ARHGAP24, XIST, IKZF1.
...
Strongest known-program match: cell_cycle (LAM-shared; overlap=2). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
