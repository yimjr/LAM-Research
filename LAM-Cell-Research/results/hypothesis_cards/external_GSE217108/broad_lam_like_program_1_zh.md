...
在 `broad_lam_like` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：MALAT1, KYNU, PTEN, FOXP1, RNF149, FAM49A, STK17B, RAB31, UTRN, USP15, MAML3, SRGAP1。
...
最强的已知程序匹配：MDK_dormancy_persistence (treatment-associated; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
