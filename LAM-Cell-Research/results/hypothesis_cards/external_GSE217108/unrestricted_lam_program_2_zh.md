...
在 `unrestricted_lam` 候选池的 pooled NMF 中出现了 `program_2`。当前排名靠前的基因为：TAF1D, NCOA7, CDC42SE2, KLF12, MRC1, EIF4G3, GAB2, SLC11A1, LRBA, MAPK1, CAMK2D, MTRNR2L8。
...
最强的已知程序匹配：hypoxia_stress (LAM-shared; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
