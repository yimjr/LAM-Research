# LAM Research Hypothesis Card: high_confidence / program_6
...
`program_6` emerged in pooled NMF of the `high_confidence` candidate pool. Leading genes are: NAV2, KIF5B, GAB2, FAM13A, NEDD9, PAM, ABCC1, MYH9, JARID2, RTN4, RPL3, FRMD4A.
...
Strongest known-program match: normal_lung_interstitial (lineage/reference; overlap=2). Partial overlap does not automatically invalidate a candidate; test whether it is a new LAMCORE implementation of a known state, has a LAM-specific component, or adds a new TF/regulon.
