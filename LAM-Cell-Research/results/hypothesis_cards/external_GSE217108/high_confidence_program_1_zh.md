...
在 `high_confidence` 候选池的 pooled NMF 中出现了 `program_1`。当前排名靠前的基因为：KIF5B, ARL15, MCTP1, CCDC141, TFDP2, SASH1, RBM47, PLEKHA1, AP3B1, DOCK5, MAP2K1, FAR1。
...
最强的已知程序匹配：lineage_uterine_smooth_muscle (lineage/reference; overlap=1)。部分重叠不会自动淘汰该候选；还需要判断它是否是已知状态在 LAMCORE 中的新实现、是否有 LAM-specific 基因或新的 TF/regulon。
