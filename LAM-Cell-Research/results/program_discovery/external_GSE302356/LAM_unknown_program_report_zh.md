# LAMCORE 未知状态程序发现报告：GSE302356

> 本报告记录的是 GSE302356 中的程序发现候选，不是独立验证完成后的新生物学结论。

运行模式：`configured_full`。当前使用配置中的完整 rank、seed 和 donor-wise 分析参数。

## 当前完成内容

- 输入：23759 个细胞、38224 个基因；
- 候选池：高置信 2681，宽松 8727，unrestricted guardrail 23759；
...
- `broad_lam_like`：选择 rank=5、seed=0，factor stability=1.000。
- `unrestricted_lam`：选择 rank=5、seed=0，factor stability=1.000。
