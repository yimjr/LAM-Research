...
Stage 24 最终采用的叙事边界是：State15 为当前最 LAM-rich 的 provisional reference-anchor candidate；Stage20 的 global manifold 结论经 Stage21 限定为不稳健统一 manifold；修正后的 Stage22 仍保留四个局部方向，但全部按 `ordinary_lineage_adjacency` 或探索性邻接解释，State15→State16 的旧 transition candidate 标签已撤回。历史版本仍在对应 Stage 报告和 `narrative_audit.csv` 中保留。
