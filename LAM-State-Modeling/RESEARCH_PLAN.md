...
修正后仍选出 4 条主要方向：State16（1/2/3-hop=96/234/65，1-hop 覆盖 8 patients、全 state 覆盖 10 patients，4 datasets）、State12（22/165/327）、State20（22/320/384）和 State7（11/87/308）；State18 未达到直接连接筛选条件。State16 的 real/null 均限制在 local 1–3 hop，并按 patient×dataset、细胞数和五档 distance 结构匹配；independent LAMCORE slope=-0.023931，直接经验双侧 p=0.878244、BH q=0.878244。State16 的 7/7 个有足够细胞的患者 slope 为负、LOPO 为 10/10 负，但仍未超过 local null，因此旧的 `LAM_to_lineage_transition_candidate` 标签撤回。State12、State20、State7 的 raw p/q 分别为 0.027944/0.055888、0.003992/0.015968、0.211577/0.282102，均不升级为 LAM branch。Boundary 本次只投射 11,648 个 local 1–3-hop cells，其中 6,736 个 unresolved；远端 boundary 不投射。Stage 22 修正后 checkpoint 为 `ordinary_lineage_adjacency_dominates`，后续不再把 State15→State16 称为 transition candidate。
