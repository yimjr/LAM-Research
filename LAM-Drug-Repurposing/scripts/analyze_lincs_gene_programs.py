...
It reads Level 5 GCTX values for the 29 concordant compounds and the genetic
...
def load_panel(signature_path: Path, n_genes: int = 150) -> pd.DataFrame:
    signatures = pd.read_csv(signature_path)
    group = signatures.loc[signatures["contrast"].eq("tsc2_loss_plastic")].copy()
    if group.empty:
        raise ValueError("tsc2_loss_plastic is absent from the GSE179044 signature file")
...
def write_report(out_dir: Path, summary: pd.DataFrame, module_matches_df: pd.DataFrame, target_df: pd.DataFrame, common_genes: set[str]) -> None:
    ...
    (out_dir / "LINCS_gene_program_analysis.md").write_text("\n".join(lines))


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--signature", default="results/signatures/GSE179044_cmap_query_signatures.csv")
    parser.add_argument("--compounds", default="results/candidates/tsc2_loss_plastic_replicated_concordant_compounds_29_unique.csv")
    parser.add_argument("--targets", default="data/processed/candidate_analysis/drug_targets/tsc2_loss_plastic_replicated_concordant_compound_targets.csv")
    ...
    panel = load_panel(ROOT / args.signature)
    ...
    summary.to_csv(out_dir / "LINCS_drug_gene_response_summary.csv.gz", index=False, compression="gzip")
    for dataset in DATASETS:
        for scope in ("reversal_only", "mimic_only"):
            matrix = make_matrix(summary.loc[summary.dataset.eq(dataset)], "direction_stability", "compound", scope)
            matrix.to_csv(out_dir / f"{dataset}_{scope}_drug_gene_direction_stability.csv")
    gene_comparison.to_csv(out_dir / "LINCS_cross_release_common_gene_comparison.csv.gz", index=False, compression="gzip")
    cluster_table.to_csv(out_dir / "LINCS_drug_gene_clusters.csv", index=False)
    matches.to_csv(out_dir / "LINCS_cross_release_module_matches.csv", index=False)
    posthoc.to_csv(out_dir / "LINCS_posthoc_module_overlap.csv", index=False)
    target_analysis.to_csv(CANDIDATE_VALIDATION / "LINCS_lestaurtinib_qlx138_target_axis_analysis.csv", index=False)
    pd.concat(mappings, ignore_index=True).to_csv(CANDIDATE_AUDIT / "LINCS_gene_panel_mapping_audit.csv", index=False)
    pd.concat(audits, ignore_index=True).to_csv(CANDIDATE_AUDIT / "LINCS_gene_analysis_signature_audit.csv", index=False)
    manifest = {
        "primary_panel": "tsc2_loss_plastic top150 up + top150 down",
    ...
    (ROOT / "manifests" / "LINCS_gene_program_analysis_manifest.json").write_text(json.dumps(manifest, indent=2))
    write_report(CANDIDATE_ANALYSIS_REPORTS, summary, matches, target_analysis, common_genes)
